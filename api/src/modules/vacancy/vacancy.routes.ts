...
const vacancyRoutes: FastifyPluginAsync = async (app) => {
  ...
  r.route({
    ...
    handler: async (req) => {
      const created = await Vacancy.create(req.body);
  ...
  r.route({
    ...
    handler: async (req, reply) => {
      const updated = await Vacancy.findByIdAndUpdate(req.params.id, req.body, { new: true }).lean();
  ...
  r.route({
    ...
    handler: async (req, reply) => {
      const res = await Vacancy.findByIdAndDelete(req.params.id).lean();
