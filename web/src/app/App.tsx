...
export default function App() {
  ...
  return (
    <div className="min-h-dvh grid grid-cols-1 lg:grid-cols-[auto_1fr]">
      <SkipLink />
      {/* SIDEBAR DESKTOP */}
      <aside
        className={`hidden lg:block p-4 border-r border-[--color-border] dark:border-zinc-800 transition-[width] duration-200 ${collapsed ? 'w-[72px]' : 'w-[240px]'}`}
        aria-label="Barra lateral"
      >
        <div className={`px-2 pb-4 text-lg font-semibold ${collapsed ? 'text-center px-0' : ''}`}>
      ...
      {/* OFF-CANVAS MÓVIL */}
      <div
        className={`lg:hidden fixed inset-0 z-[100] ${mobileOpen ? '' : 'pointer-events-none'}`}
        aria-hidden={!mobileOpen}
      >
        <div
          className={`absolute inset-0 bg-black/40 transition-opacity z-[100] ${mobileOpen ? 'opacity-100' : 'opacity-0'}`}
          onClick={() => setMobileOpen(false)}
        />
        <aside
          className={`absolute left-0 top-0 h-full w-[84%] max-w-[320px] p-4
          bg-[--color-bg] dark:bg-zinc-900
          text-[--color-fg] dark:text-zinc-100       /* <-- forzamos contraste */
          border-r border-[--color-border] dark:border-zinc-800
          transition-transform will-change-transform z-[110]
        ...
        >
          ...
          <nav className="flex flex-col gap-1">
            {NAV.map(item => (
              <SidebarItem key={item.to} to={item.to} icon={item.icon} collapsed={false}>
                {item.label}
              </SidebarItem>
            ))}
          </nav>
          <button className="btn btn-primary mt-4 w-full" onClick={() => setMobileOpen(false)} aria-label="Cerrar menú">
            Cerrar
          </button>
        </aside>
      </div>
      ...
      <div className="flex flex-col min-h-dvh">
        <Topbar onOpenHistory={() => setOpenHistory(true)} onToggleSidebar={() => setMobileOpen(true)} />
